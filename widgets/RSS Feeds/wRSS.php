	...
	foreach ($settingsDB as $setting) {
		if ($setting['Widget'] == 'wRSS' ) {
			if ($setting['Id'] == 'rssfeeds') {
				...
				if (!empty($rssfeeds)) {
					echo "\t<strong>RSS Feeds:</strong><br />";
					foreach ($rssfeeds as $rssfeed){
						echo "\t\t<strong>".$rssfeed['label']."</strong><br/>";
						echo "\t\tName: <input type='text' value='".$rssfeed['label']."' name='rssfeed-".$i."-label'  />";
						...
						echo "\t\tScript: <input type='text' value='".$rssfeed['script']."' name='rssfeed-".$i."-script'  />";
						echo "\t\tDel: <input type='checkbox' name='rssfeed-".$i."-remove' value='true' /><br /><br />\n";
						$i++;
					}
				}
				echo "\t<strong>Add RSS Feed:</strong><br />";
				echo "\t\tName: <input type='text' value='' name='addfeed-".$i."-label'  /><br /><br />\n";
			}
		} 
	}
...
function wRSSUpdateSettings($post) {
	...
	foreach ($post as $id => $value) {
		...
		if (strpos($id, 'addfeed') !== false) {				
			if (strpos($id, 'label') !== false) {
				if (!empty($value)) {
					$rssfeeds["rssfeed".$i]['label'] = $value;
					$rssfeeds["rssfeed".$i]['type'] = "";
					$rssfeeds["rssfeed".$i]['url'] = "";
					$rssfeeds["rssfeed".$i]['category'] = "";
					$rssfeeds["rssfeed".$i]['priority'] = "";
					$rssfeeds["rssfeed".$i]['postprocess'] = "";
					$rssfeeds["rssfeed".$i]['script'] = "";
				} else {
					$post['rssfeed-'.$i.'-remove'] = 'true';
				}
				$i++;
			}
		} elseif (strpos($id, 'rssfeed') !== false) {	
			if (strpos($id, 'label') !== false) {
				$rssfeeds["rssfeed".$i]['label'] = $value;			
				...
				if (!isset($post['rssfeed-'.$i.'-remove'])){
					$i++;		
				}
			} elseif (strpos($id, 'remove') !== false) {
				if ($value == 'true') {
					unset($rssfeeds["rssfeed".$i]);
				}
				$i++;		
			}	
		}	 
