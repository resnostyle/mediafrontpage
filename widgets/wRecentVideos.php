<?php
$wdgtRecentMovie = array("type" => "ajax", "block" => "recentmoviewrapper", "call" => "widgets/wRecentVideos.php?ajax=w&t=m&a=l&c=15", "interval" => 0, "headerfunction" => "widgetRecentMovieHeader(\$params);");
$wIndex["wRecentMovies"] = $wdgtRecentMovie;

$wdgtRecentTV = array("type" => "ajax", "block" => "recenttvwrapper", "call" => "widgets/wRecentVideos.php?ajax=w&t=t&a=l&c=15", "interval" => 0, "headerfunction" => "widgetRecentTVHeader(\$params);");
	...
	echo <<< RECENTTMOVIEHEADER
		...
		<!--
			function cmdRecentMovie(action, param) {
				...
				var request = "widgets/wRecentVideos.php?ajax=w&t=m&a=l&c="+param;
				switch(action) {
					case "p":
	...
	echo <<< RECENTTVHEADER
		...
		<!--
			function cmdRecentTV(action, param) {
				...
				var request = "widgets/wRecentVideos.php?ajax=w&t=t&a=l&c="+param;
				switch(action) {
					case "p":
		...
		if (!empty($videos)) {
			switch ($action) {
				case "d":  // Display
					...
					break;
				case "p":  // Play
					playvideofromlist($videos, $videoId, "episodeid"); 
					//foreach ($videos as $episodeInfo) {
					//	if($videoId == $episodeInfo['episodeid']) {
				...
				default: //Default to l - List
					echo "<ul>";
					foreach ($videos as $episodeInfo) {
						$display = $episodeInfo['showtitle']." - ".$episodeInfo['season']."x".str_pad($episodeInfo['episode'], 2, '0', STR_PAD_LEFT)." - ".$episodeInfo['label'];
		...
		if (!empty($videos)) {
			switch($action) {
				case "d":  // Display
					...
					break;
				case "p":  // Play
					playvideofromlist($videos, $videoId, "movieid"); 
					//foreach ($videos as $movieInfo) {
					//	if($videoId == $movieInfo['movieid']) {
				...
				default: //Default to l - List
					foreach ($videos as $movieInfo) {
						$display = $movieInfo['label']." &nbsp;(".$movieInfo['year'].")";
